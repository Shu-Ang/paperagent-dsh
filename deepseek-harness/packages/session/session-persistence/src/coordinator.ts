...
import {
  adoptSessionEvent,
  decodeRegisteredSessionEventData,
...
function snapshotStoredEvents(events: readonly SessionEvent[], id: SessionId): SessionEvent[] {
  ...
  return events.map((event) => {
    const decoded = decodeRegisteredEvent(event)
    const migratedStart = migrateLegacyTurnStartEvent(decoded, id)
...
/** Apply a plugin-owned payload decoder before legacy migration and replay. */
function decodeRegisteredEvent(event: SessionEvent): SessionEvent {
  const data = decodeRegisteredSessionEventData(event.type, event.data)
  return data === event.data ? event : { ...event, data } as SessionEvent
}
