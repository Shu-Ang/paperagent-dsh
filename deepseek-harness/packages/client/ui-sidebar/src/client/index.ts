...
export function apply(ctx: ClientContext): void {
  ...
  const injectProps = (): SidebarRootInjected => ({
    ...
    startSession: (workspaceId) => {
      // New Session is always a Chat action. Business main views own no
      // session state, so returning to Chat before opening preserves the
      // runtime's ordinary session creation flow.
      ctx.layout.activateMainView(null)
