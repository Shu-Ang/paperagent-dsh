...
describe('ui-layout client apply', () => {
  ...
  it('injects the generic main-view controller and attaches the layout actions', async () => {
    const { ctx, slots } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    const actions = {
    ...
    expect(injected).toEqual({ mainViews: expect.any(LayoutController) })
    const layout = ctx.get('layout') as LayoutController
    layout.toggleSidebar()
    expect(actions.toggleSidebar).toHaveBeenCalledOnce()
    const unregister = layout.registerMainView('example')
    expect(layout.activateMainView('example')).toBe(true)
    expect(layout.getMainView()).toBe('example')
    unregister()
    expect(layout.getMainView()).toBeNull()
  })
