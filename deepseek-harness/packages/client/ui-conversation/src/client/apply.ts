...
import {
  resolveWorkspacePath, type ConversationLocationDataStore, type ConversationTurnDataMap, type ISessions, type SessionId,
...
import { ChatInlineCodeMentions, combineMarkdownFileMentions } from './chat/inline-code-mentions.ts'
...
type SubscribableTurnDataStore = ConversationLocationDataStore<ConversationTurnDataMap> & {
  getSnapshot(): number
  subscribe(listener: () => void): () => void
}

// The runtime package's generated declaration may lag one build behind while
// the host is being rebuilt. Keep this boundary local; the source contract
// itself declares the methods and the runtime object implements them.
const ABSENT_TURN_DATA = {
  get: () => undefined,
  getSnapshot: () => 0,
  subscribe: () => () => {},
} as unknown as SubscribableTurnDataStore

const CHAT_NODE_INJECT: ChatNodeTurnDataInjected = {
  hooks: {
    turnData: ({ useSession }, nodeKey) => function useTurnData(key) {
      const store = useSession((snapshot) => {
...
export function apply(ctx: Context): void {
  const inlineCodeMentions = new ChatInlineCodeMentions()
  ctx.provide('chatInlineCodeMentions', inlineCodeMentions)
  ...
  slots.register({
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
      ...
      return {
        ...
        fileMentions: owner => combineMarkdownFileMentions([
          ...[ctx.get('chatFileMentions')?.forClosing(owner)].filter((value): value is NonNullable<typeof value> => value !== undefined),
          ...[inlineCodeMentions.forClosing(owner)].filter((value): value is NonNullable<typeof value> => value !== undefined),
        ]),
        openFile: (path) => {
