...
/** One owner-validated token rendered as an optional inline action. */
export interface MarkdownMention {
  /** Runs the owner-authorized navigation or action. */
  open: () => void
  /** Accessible description of the action. */
  label: string
  /** Native hover description. */
  title: string
  /** Replacement display text; the authored token is the fallback. */
  text?: string
  /** Renders as informational text when the target is unavailable. */
  unavailable?: boolean
}

/** A validated token range in one ordinary Markdown text node. */
export interface MarkdownTextMention {
  /** Zero-based inclusive start in the text node. */
  start: number
  /** Zero-based exclusive end in the text node. */
  end: number
  /** The owner-authorized replacement and action. */
  mention: MarkdownMention
}

/**
 * File-mention affordance for inline code, with an optional ordinary-prose
 * token matcher. The owner resolves authored text using its own vocabulary;
 * the renderer never guesses at paths, citations, or other identifiers.
 */
export interface MarkdownFileMentions {
   ...
   */
  resolve(value: string): MarkdownMention | undefined
  /**
   * Resolve validated token ranges in an ordinary text node. Used by features
   * whose model protocol may omit Markdown code delimiters. Returned ranges
   * must be non-overlapping and are defensively checked by the renderer.
   */
  resolveText?(value: string): readonly MarkdownTextMention[]
}
...
/** Render owner-validated tokens in normal prose without introducing code chrome. */
function renderTextMentions(value: string, key: Key, context: MarkdownRenderContext): ReactNode {
  if (context.inLink === true) return value
  const candidates = context.fileMentions?.resolveText?.(value)
  if (candidates === undefined || candidates.length === 0) return value

  const mentions = [...candidates]
    .filter(item => Number.isSafeInteger(item.start)
  ...
  let offset = 0
  for (const [index, item] of mentions.entries()) {
    // Overlaps are ignored rather than letting independently registered
    // vocabularies rewrite one another's text.
    if (item.start < offset) continue
    if (item.start > offset) children.push(value.slice(offset, item.start))
    children.push(renderMention(item.mention, value.slice(item.start, item.end), `${String(key)}-mention-${index}`, false))
    offset = item.end
  }
  if (offset === 0) return value
  if (offset < value.length) children.push(value.slice(offset))
  return <Fragment key={key}>{children}</Fragment>
}

/** Render a mention either inside authored code or directly in ordinary prose. */
function renderMention(mention: MarkdownMention, fallback: string, key: Key, codeChrome: boolean): ReactNode {
  const content = mention.text ?? fallback
  if (mention.unavailable === true) {
    const inert = <span title={mention.title}>{content}</span>
    return codeChrome ? <code key={key}>{inert}</code> : <Fragment key={key}>{inert}</Fragment>
  }
  const action = (
    <button
      type="button"
  ...
  return codeChrome ? <code key={key}>{action}</code> : <Fragment key={key}>{action}</Fragment>
}

function renderNode(node: Md.RootContent, key: Key, context: MarkdownRenderContext): ReactNode {
  switch (node.type) {
    case 'text':
      return renderTextMentions(node.value, key, context)
    case 'paragraph':
    ...
    case 'inlineCode': {
      ...
      if (mention !== undefined) return renderMention(mention, value, key, true)
